    ...
    public function index() {
        ...
        if($this->cart->total_items() <= 0) {
            ...
            if($this->form_validation->run() == true) { 
                ...
                if($order) {
                    $this->session->set_flashdata('success_msg', 'Terima kasih! Pesanan Anda telah berhasil dibuat!');
                        redirect(base_url().'orders');
                } else {
                        $data['error_msg'] = "Pengiriman pesanan gagal, harap coba lagi.";
                }
            }

        $data['user'] = $user;
        $data['cartItems'] = $this->cart->contents();
        $judul['title'] = 'Checkout';
